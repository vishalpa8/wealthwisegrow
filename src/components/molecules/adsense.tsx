...
export const AdSense: React.FC<AdSenseProps> = ({ adSlot, style, className }) => {
  ...
  if (process.env.NODE_ENV === 'development') {
    return <div className='w-full h-full bg-gray-100 flex items-center justify-center text-gray-400 border border-dashed border-gray-300 rounded'>Advertisement</div>;
  }
    ...
    >
      {isLoading && (
        <div className="absolute inset-0 z-10">
          <div className='w-full h-full bg-gray-100 flex items-center justify-center text-gray-400 border border-dashed border-gray-300 rounded'>Advertisement</div>
        </div>
      )}
      <ins
        ref={adRef}
